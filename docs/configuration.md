...
Внедряется ADR [0001](adr/0001-configurable-product.md). Реестр видов, валидация реквизитов, проекция хранилища и выбор транзакционной операции теперь загружаются извне. Переход всего продукта ещё не завершён: React переведён на schema/ui; системный запуск двух заказчиков на реальном Platform V ещё не проверен. Это не заявление о выполнении полного proof of configurability.
