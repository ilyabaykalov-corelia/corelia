...
Внедряется ADR [0001](adr/0001-configurable-product.md). Реестр видов, валидация реквизитов, проекция хранилища и выбор транзакционной операции теперь загружаются извне. Переход всего продукта ещё не завершён: React переведён на schema/ui; проверка действующей поставки СберНПФ на реальном Platform V ещё не выполнена. Это не заявление о выполнении полного proof of configurability.
...
По уточнению заказчика от 17.09.2026 реальная конфигурация одна: СберНПФ, ПДС и КИД ОПС. Два синтетических каталога служат регрессии расширяемости; отдельные платформенные поставки для них не требуются. Дополнительные logical actions вне текущих процессов не являются обязательным расширением этой миграции.

`sber-npf-corelia-config/scripts/package-solution.py` собирает runtime и полный платформенный архив из скомпилированного пакета и манифеста платформы. Проверяет checksums, одинаковые права, состав видов и наличие процессов; сохраняет остальные операции и условия permissions. Подробная команда — в README пакета СберНПФ. Это локальная сборка, без публикации и автоматической миграции данных.
