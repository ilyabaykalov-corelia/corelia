...
Срез локальных исходников: 16 сентября 2026 года. Обновлено при переходе к внешней конфигурации; подробности и ограничения — в [configuration.md](configuration.md). Этот документ объединяет реализованные механизмы; результаты старых проверок не выдаются за новую проверку.

## Компоненты

| Каталог | Что реализовано и где начинать чтение |
| --- | --- |
| `corelia-configuration` | Независимый загрузчик, строгий профиль schema и immutable registry |
| `corelia-config-compiler` | Проверка и упаковка runtime metadata, операций, permissions fragment и fixtures |
...
`DocumentVersionService` реализует общий алгоритм, `ConfiguredDocumentPolicy` по внешнему registry — валидацию и проверку прав вида. Репозитории отделяют алгоритм от операций DataSpace. Текущая версия схемы обоих видов — 1; автоматическое преобразование неизвестных исторических схем не реализовано.
...
Разрешённые тексты GraphQL в `sber-npf-corelia-config/graphql` должны соответствовать `../sber-npf-platform-v/model.graphql-permissions.json` и независимой тестовой фикстуре `corelia-system-tests/src/test/resources/platform-v/allowed-requests.json`. Проверка имени операции без проверки тела недостаточна.
...
Для нового вида backend использует внешний schema/UI/storage package; модель, статические операции и permissions подготавливаются отдельно. Политика валидации и проекция больше не требуют Java-класса вида. React, авторизация и workflow пока мигрированы не полностью. Отдельный микросервис и отдельная таблица версий на вид не требуются. Правила ПДС нельзя считать общими для всех видов.
