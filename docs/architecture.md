...
Общие поля карточки хранятся в `Document`, текущие реквизиты ПДС и его `status` — в дочернем `PdsContract`. Общий `DocumentVersion` хранит JSON-снимки атрибутов и состава вложений, а `DocumentCommand` — результаты повторяемых команд. Правила версий принадлежат document-service. `Attachment` хранит версии файлов независимо; состав закрытой версии документа не меняется. Подробности: [версии документов](document-versioning.md).
